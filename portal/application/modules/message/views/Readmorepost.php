                      ...
                      <div class="card w-100 mb-1" style="min-height: 400px;">
                            <?php if($GetSpecifiPost->num_rows() > 0): ?>
                              <?php foreach($GetSpecifiPost->result() as $GetSpecifiPost_row):
                                        $TimelineID = $GetSpecifiPost_row->message_id;
                              ...
                              <div class="card-header bNotification-transparent">
                                  <h3 class="card-title"> <?= $GetSpecifiPost_row->title ?></h3>
                                    <div class="ml-auto clearfix float-right">
                                          <?= date('M d, Y : h:i  ', strtotime( $GetSpecifiPost_row->created_at)); ?>
                                    </div>
                              </div>
...
<form id="commentForm" class="comment-form" method="POST" action="<?= site_url('message/Messages/Reply_Message'); ?>">
<input type="hidden" name="message_id" value="<?= $GetSpecifiPost_row->message_id  ?>">
 <input type="hidden" name="reciever_id" value="<?= $GetSpecifiPost_row->sender_id  ?>">
      ...
      <?php endif; ?>
          <div class="container-fluid mt-3 mb-5 ">
                ...
                <div class="comment-content w-75">
                  <div class="commenter-head"><span class="commenter-name"><a href="" ><?= $commentshow_row->email ?>  </a></span> <span class="comment-date"><i class="far fa-clock"></i><?= date('M d, Y : h:i  ', strtotime($commentshow_row->created_at)); ?></span></div>
